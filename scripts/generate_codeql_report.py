...
def wait_for_alerts_processed(max_wait_seconds=90):
    ...
    while time.time() < deadline:
        ...
        time.sleep(5)
    print(
        f"Gave up waiting after {max_wait_seconds}s — reporting with latest available data."
    )
...
def severity_of(alert):
    rule = alert.get("rule", {})
    return (
        rule.get("security_severity_level") or rule.get("severity") or "note"
    ).lower()
...
def build_pdf(alerts, out_path):
    ...
    status_color = (
        colors.HexColor("#2e7d32") if not alerts else colors.HexColor("#c62828")
    )
    ...
    story.append(
        Paragraph(
            (
                "No open alerts"
                if not alerts
                else f"{len(alerts)} open alert{'s' if len(alerts) != 1 else ''}"
            ),
            status_style,
        )
    )
    ...
    if not alerts:
        ...
        alerts_sorted = sorted(
            alerts, key=lambda a: SEVERITY_ORDER.get(severity_of(a), 9)
        )
