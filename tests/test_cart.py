...
class TestCartView:
    ...
    def test_cart_with_items_computes_line_and_total_correctly(
        ...
        mock_db["query_all"].return_value = [
            {"product_id": 1, "ip_address": "127.0.0.1", "quantity": 3}
        ]
        mock_db["query_one"].return_value = {
            "product_id": 1,
...
class TestUpdateQuantity:
    ...
    def test_update_rejects_missing_csrf(self, logged_in_customer, mock_db):
        resp = logged_in_customer.post(
            "/cart", data={"update_cart": "1", "quantity_1": "2"}
        )
        assert resp.status_code == 403


class TestRemoveFromCart:
    def test_remove_selected_deletes_each_checked_row(
        self, logged_in_customer, mock_db
    ):
        mock_db["query_all"].return_value = [
            {"product_id": 1, "ip_address": "127.0.0.1", "quantity": 1}
    ...
    def test_remove_with_nothing_checked_shows_prompt(
        self, logged_in_customer, mock_db
    ):
        mock_db["query_all"].return_value = [
            {"product_id": 1, "ip_address": "127.0.0.1", "quantity": 1}
