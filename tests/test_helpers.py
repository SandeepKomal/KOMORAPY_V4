...
class TestSaveUpload:
    def test_content_detection_overrides_a_lying_filename(self, tmp_path):
        ...
        buf.seek(0)
        upload = FileStorage(
            stream=buf, filename="fake.jpg"
        )  # says .jpg, is really PNG

        result = save_upload(upload, str(tmp_path))
        assert result is not None
        assert result.endswith(
            ".png"
        ), f"expected real content (.png) to win over the lying filename, got {result}"
...
class TestCsrf:
    def test_token_is_generated_and_persisted_in_session(self, app):
        ...
        with app.test_request_context():
            ...
            assert (
                token1 == token2
            )  # same session -> same token, not regenerated each call
    ...
    def test_check_accepts_matching_token(self, app):
        from app.helpers import csrf_check, csrf_token
...
class TestSafeRedirectTarget:
    ...
    def test_external_domain_is_rejected(self, app):
        ...
        with app.test_request_context("/", base_url="https://example.com"):
            result = safe_redirect_target(
                "https://evil-phishing-site.com/fake-login", "/fallback"
            )
